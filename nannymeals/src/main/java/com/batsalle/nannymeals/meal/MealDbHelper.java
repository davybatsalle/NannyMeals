...
public class MealDbHelper extends SQLiteOpenHelper {
    ...
    public void removeItems() {
        this.initWritable();
        this.writableDb.delete(MealContract.MealTable.TABLE_NAME, null, null);
    }


    public List<MealStats.MealStatItem> retrieveStats(String fieldName, Integer nbMonths) {
        ...
        this.initReadable();
        String query = "select " + fieldName + ",count(*) as counter from " +
                MealContract.MealTable.TABLE_NAME +
                " where " + MealContract.MealTable.COLUMN_DATE + "> \"" + date + "\"" +
                " and length(" + fieldName + ") > 0" +
                " group by " + fieldName +
                " order by counter desc";

        Cursor cursor = this.readableDb.rawQuery(query,null);
        List<MealStats.MealStatItem> statList = new ArrayList<>();

        if (cursor != null) {
            if (cursor.moveToFirst()) {
                do {
                    MealStats.MealStatItem item = new MealStats.MealStatItem();
                    item.category = fieldName;
                    item.counter = cursor.getInt(cursor.getColumnIndex("counter"));
                    item.value =   cursor.getString(cursor.getColumnIndex(fieldName));
